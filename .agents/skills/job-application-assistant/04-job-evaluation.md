...
**Strong match areas:** Terraform / Terragrunt IaC, Kubernetes (bare-metal and cloud), Helm, GitOps CI/CD (ArgoCD, Flux, Kargo, GitLab CI/CD, GitHub Actions), observability (Prometheus, Grafana, Loki, Tempo, Mimir, Thanos, OpenTelemetry), AWS infrastructure and cost optimisation, Ansible, Linux
**Moderate match areas:** Go and Python (working level), GCP, OVH Cloud, PostgreSQL tuning and replication, ScyllaDB / Redis / InfluxDB, Nginx / HAProxy, Jenkins, Kustomize, security and compliance (zero-trust, mTLS, RBAC, network policies, SOC-2)
**Weak match areas:** Node.js / TypeScript (basic only), Azure (not in profile), service mesh (Istio / Linkerd not in profile), pre-sales and customer-facing solution design (matters for Solutions Architect roles), active certifications (AWS SAA and CKA both expired 2026), formal people management (led a team without the title)
...
**Strong:** DevOps / infrastructure engineering for production systems - IaC, Kubernetes platforms, CI/CD, observability, AWS cost reduction, SOC-2 compliance (4+ years CV-visible, all titled DevOps Engineer)
**Moderate:** SRE-style work (P99 latency, high-load DNS, observability) without an SRE title; Observability Engineering (designed a self-hosted LGTM stack); platform engineering (internal developer platform at Goya); team leadership (led the Goya DevOps team informally)
**Entry-level:** Solutions Architect (strong architecture design work, but no architect title, no pre-sales or customer-facing design experience in the profile, AWS SA certification expired 2026). Technical-design SA roles fit better than pre-sales-heavy ones - the DISC profile shows very low "Inspiring" (persuasion, selling); see `02-behavioral-profile.md`.
...
The candidate is based in Yerevan, Armenia (UTC+4) and accepts **fully remote** work only.
- Fully remote and open to candidates in Armenia: PASS (an Armenian company is a stated preference - note it as a plus)
- Fully remote but the posting is silent on which countries it hires from: PROCEED, mark unverified - check the careers page before drafting
- Remote but limited to a region or time-zone band ("EMEA", "CET +/- 2h"): PASS if Armenia / UTC+4 fits; FLAG if Armenia's eligibility is unclear or US-hours overlap is required
- Remote but limited to countries that exclude Armenia ("US only", "EU residents only", "must be based in the UK"): FAIL
- Hybrid (any regular office days): FLAG - not the stated preference, discuss with user
- On-site: FAIL (deal-breaker)
...
**Career goals:**
- Senior DevOps Engineer in a fully remote team (primary target)
- Observability Engineer - build on the self-hosted LGTM / OpenTelemetry work
- Solutions Architect - design infrastructure end to end, not only operate it

**Motivation filter:** Evaluate not just whether you *can* do the tasks, but whether the tasks will *energize* you. Consider:
- Tasks that energize: *[Inferred from work history - not yet confirmed]* building platforms from zero, replacing manual work with automation, self-hosting to replace paid SaaS, measurable cost and latency wins
- Tasks that drain: not yet stated
- Non-task factors: leadership style, department culture, company values, degree of autonomy

**Life situation alignment:** Consider personal constraints:
- **Security**: Employed at CodedCloud and actively looking; 2-week notice period. Salary baseline 1,500,000 AMD/month or more, net (in hand). Below baseline is a note, not a deal-breaker.
- **Flexibility**: Fully remote only; on-site is the one hard deal-breaker.
- **Professional development**: Senior-level scope. Renewing AWS SAA and CKA (both expired 2026) would strengthen Solutions Architect and Kubernetes-heavy applications.
