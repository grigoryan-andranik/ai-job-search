...
- **`/price`** researches what to ask a specific company to pay. It gathers fetched evidence (the posting's own range, the company's published pay, market data for the role, level and pay geography), sets a walk-away / target / opening-ask range adjusted for your fit, and converts it into AMD, USD and EUR, monthly and yearly, at today's Central Bank of Armenia rate via `tools/fx_convert.py`. Net and gross are never mixed silently, and every figure traces to a source. The report is saved into the application's archive folder.
