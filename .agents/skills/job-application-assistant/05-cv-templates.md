...
% Link text is the literal handle, not "LinkedIn"/"GitHub", so the URLs survive in the ATS text layer.
...
**For Senior DevOps / Platform Engineering roles (primary):**
> Senior DevOps engineer with 4+ years automating and operating high-load production infrastructure. Deep expertise in IaC (Terraform / Terragrunt), Kubernetes, GitOps-driven CI/CD, observability (LGTM stack + OpenTelemetry) and PostgreSQL performance tuning. Proven track record of eliminating manual toil and cutting cloud costs by 50%.

*[Source: candidate's own CV summary, grounded against `01-candidate-profile.md`: "5+ years" changed to "4+ years" (CV-visible history starts Feb 2022) and "50%+" changed to "50%" (the underlying bullet says 50%). "Senior" is target-level positioning the candidate confirmed on 2026-09-11; every title held is "DevOps Engineer", so never write "Senior" into a `\cventry` title. "SRE" is not claimed as a past title - "SRE-style work" is fine when a posting asks for it.]*

**For Observability Engineer roles:**
> Senior DevOps engineer (4+ years) with a focus on observability. Designed a self-hosted LGTM stack (Grafana, Loki, Mimir, Tempo) with OpenTelemetry that handles 400 GB of logs and metrics per day and replaced a paid SaaS tool, cutting its cost by 75%. Hands-on with Prometheus, Alertmanager and Thanos, and with latency-critical services, including a self-hosted DNS platform serving 120M+ requests per day at under 80 ms P99.

**For Solutions Architect roles:**
> Senior infrastructure engineer with 4+ years designing platforms end to end: bare-metal Kubernetes from zero to production, a zero-trust internal developer platform, and a full Terraform migration with drift detection. Works across AWS, GCP and OVH Cloud, and pairs architecture with ownership of cost (50% AWS reduction), compliance (owned the infrastructure side of a SOC-2 Type II audit) and delivery at a B2B IT consultancy.

*Gap to handle honestly:* Solutions Architect postings usually ask for pre-sales or customer-facing design experience and an active AWS Solutions Architect certification. Neither is in the profile (SAA-C03 expired 2026) - acknowledge it in the cover letter rather than stretch the CV.
